...
namespace KartGame.KartSystems
{
    public class ArcadeKart : MonoBehaviour
    {
        ...
        private InputData inputMLAgent;
        [SerializeField] bool isML;


        void Start()
        {
            inputMLAgent.TurnInput = -10f;
        }
        ...
        void FixedUpdate()
        {
            ...
            if (isML)
            {
                if (inputMLAgent.TurnInput != -10f)
                {
                    updatesNecessary();
                    GatherInputsMLAgent(inputMLAgent);
                    updatesFixedUpdate();
                    inputMLAgent.TurnInput = -10f;
                }
            }
        ...
        public void FixedUpdateForML(InputData input)
        {
            inputMLAgent = input;

        }

        public void updatesNecessary()
        {
            UpdateSuspensionParams(FrontLeftWheel);
            UpdateSuspensionParams(FrontRightWheel);
            UpdateSuspensionParams(RearLeftWheel);
        ...
        public void updatesFixedUpdate()
        {
